...
The system SHALL model a registry entry as `id`, `name`, `description`, `source-type` (`git` or `local`), and a `source` object shaped by `source-type`: a `git` entry's `source` SHALL have `uri` and `ref` (defaulting to `main` when omitted); a `local` entry's `source` SHALL have only `uri` (no `ref`).
